	...
	for (int i = 0; i < h->max_clients; ++i) {
		if (h->clients[i].type == CLIENT_ACTIVE) {
			uint64_t check_this_client_at =
				h->clients[i].last_seen +
				h->keep_alive_period_ms;
			if (check_this_client_at <
			    check_after_ms + _tick_get_ms()) {
				check_after_ms =
					check_this_client_at - _tick_get_ms();
				if (check_after_ms < 0) {
					check_after_ms =
	...
	while (run_task) {
		if (xQueueReceive(keep_alive_storage->q, (void *)&client_action,
			...
			case CLIENT_FD_REMOVE:
				...
				break;
			case CLIENT_UPDATE:
				if (!update_client(keep_alive_storage,
						   client_action.fd,
			...
			for (int i = 0; i < keep_alive_storage->max_clients;
			     ++i) {
				if (keep_alive_storage->clients[i].type ==
				    CLIENT_ACTIVE) {
					if (keep_alive_storage->clients[i]
							    .last_seen +
						    keep_alive_storage
...
wss_keep_alive_t wss_keep_alive_start(wss_keep_alive_config_t *config) {
	...
	wss_keep_alive_t keep_alive_storage = calloc(
		1, sizeof(wss_keep_alive_storage_t) +
			   client_list_size * sizeof(client_fd_action_t));
	if (keep_alive_storage == NULL) {
		return false;
	}
	keep_alive_storage->check_client_alive_cb =
		config->check_client_alive_cb;
    ...
    ESP_LOGI("ka", "before queue");
	keep_alive_storage->q =
		xQueueCreate(queue_size, sizeof(client_fd_action_t));
	if (xTaskCreate(keep_alive_task, "keep_alive_task",
			config->task_stack_size, keep_alive_storage,
			config->task_prio, NULL) != pdTRUE) {
		wss_keep_alive_stop(keep_alive_storage);
		return false;
	}
    ESP_LOGI("ka", "after queue");
...
esp_err_t wss_keep_alive_add_client(wss_keep_alive_t h, int fd) {
	client_fd_action_t client_fd_action = { .fd = fd,
						.type = CLIENT_FD_ADD };
